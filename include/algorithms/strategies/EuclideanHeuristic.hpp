 ...
 * 
 * Calcule sqrt((x1-x2)² + (y1-y2)² + (z1-z2)² + ...) (dimension variable)
 * 
 * USAGE: Environnements où mouvement dans toutes directions possible
 * PROPRIÉTÉS: Admissible, plus précise que Manhattan pour mouvements libres
 */
...
public:
    explicit EuclideanHeuristic(float weight = 1.0f) : weight_factor(weight) {
        if (weight < 1.0f) {
    ...
    float calculate_heuristic(const Point* from, const std::vector<float>& goal) const override {
        ...
        {
            sum+= (coords[i]-goal[i])*(coords[i]-goal[i]);
        }
